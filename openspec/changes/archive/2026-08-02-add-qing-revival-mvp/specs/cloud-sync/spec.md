...
> ⚠️ **同步策略**采用 design.md D7「服务端权威 updated_at 方案」。**POST 接口不接收 `updatedAt` 字段**（zod `.strict()` 模式拒绝），由服务端 `defaultNow()` 唯一生成，从源头消除两设备并发 POST 的 race。客户端 `updatedAt` 仅作本地排序与 UI 显示。
