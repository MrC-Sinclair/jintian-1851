...
游戏 MUST 在 `game-main` 显著位置展示玩家当前综合实力进度，让玩家随时知晓距离胜利（综合实力≥90）的差距。综合实力 = 5 维属性（军事+经济+政治+民心+外交）等权平均，取值 0-100。
...
AND `overallPower` 由 `utils/goal-hint.ts` 的 `calcOverallPower(attributes)` 计算
