...
- **THEN** 疑问求助词开头者由后端**代码层确定性拦截**（不调 LLM，响应含 `hesitation: true`），返回唯一犹豫签名 effects `{ people: -1, silver: -10 }` 且 `factionEffects` 恒为空数组（提问不得改变势力关系）；前端检测到犹豫签名时提示玩家应去问军师。"我想 + 具体动作"（如"我想暗中资助湘军"）是行动决策，不判犹豫
