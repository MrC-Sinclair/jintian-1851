...
## Purpose
服务端 `server/server/tools/` 集中维护所有工具定义（每工具一文件），经 `createTools(ctx)` 注入 `ToolContext`（saveId/turn/stateSnapshot/character/factions/recentEvents）。供军师对话与 NPC Agent 共用的工具系统，避免工具逻辑散落。
## Requirements
