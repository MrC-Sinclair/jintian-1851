 ...
 * @file 模型白名单与能力配置
 *
 * 严格复制自 my-chat/server/config/models.ts（4 模型方案）。
 *
 * 硅基流动 SiliconFlow 4 模型方案：
 *   - Qwen3-8B：默认启用思考（实测），支持 enable_thinking 开关切换，支持工具调用
 *   - DeepSeek-R1-0528-Qwen3-8B：强制思考模型，不认识 enable_thinking，不支持工具调用
 *   - GLM-Z1-9B-0414：强制思考模型，传 enable_thinking 会 400 报错，不支持工具调用
 *   - Qwen3.5-4B：原生多模态（视觉+工具调用），可通过 enable_thinking 关闭思考
 */
...
/** 允许使用的模型值集合，API 路由中用于校验请求的 model 参数 */
