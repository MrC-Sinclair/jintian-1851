...
【解析规则】
1. **第一步必须先做疑问句判定（优先于选项匹配）**：若玩家决策仅表达诉求、情绪或提问，而无任何具体行动指令，视为"决策犹豫"：
   - 必须且仅能返回：{ "effects": { "people": -1, "silver": -10 }, "factionEffects": [] }（极小"决策犹豫"代价，不消耗兵力）
   - **禁止**返回其他任何 effects 数值，**禁止**返回任何 factionEffects（玩家提问不应改变势力关系）
   - 玩家应使用"军师"按钮提问，不要用"自由行动"提问
   - 注意："我想 + 具体动作"（如"我想资助湘军""我想开仓放粮"）是行动决策，不是疑问句，应按第 2 步正常解析
2. 判断玩家决策与哪个预设选项最接近（仅当第 1 步未命中犹豫时执行）：
